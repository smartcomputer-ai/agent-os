...
- [ ] Add golden tests: multiple sugar encodings of the same params must yield identical `intent_hash`/`params_ref`; reducer-emitted canonical params must round-trip unchanged. (partial: header-order unit test in `aos-effects`; plan LLM float-vs-string canonicalization + reducer timer canonicalization integration tests in `crates/aos-testkit/tests/effect_params_normalization.rs`)
