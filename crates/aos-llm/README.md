...
```rust
use aos_llm::errors::{RetryPolicy, retry_async};
...
- Mocked OpenAI integration: `crates/aos-llm/tests/openai_integration_mocked.rs`
- Mocked Anthropic integration: `crates/aos-llm/tests/anthropic_integration_mocked.rs`
- Cross-provider conformance tests: `crates/aos-llm/tests/cross_provider_conformance.rs`
- Optional live OpenAI tests: `crates/aos-llm/tests/openai_live.rs`
- Optional live Anthropic tests: `crates/aos-llm/tests/anthropic_live.rs`

## Build

```
cargo build -p aos-llm
```
...
```
cargo test -p aos-llm
```

Run live OpenAI integration tests (ignored by default):

```
RUN_LIVE_OPENAI_TESTS=1 cargo test -p aos-llm --test openai_live -- --ignored
```
...
```
RUN_LIVE_ANTHROPIC_TESTS=1 cargo test -p aos-llm --test anthropic_live -- --ignored
```
