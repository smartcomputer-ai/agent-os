...
#[tokio::test]
async fn keyed_workflow_integration_flow() {
    ...
    let build_manifest = || {
        ...
        let mut manifest = fixtures::build_loaded_manifest(vec![workflow], routing);
    ...
    // Head view exposes both cells even before snapshot materialization.
    let metas = kernel.list_cells("com.acme/Keyed@1").unwrap();
    ...
    for meta in &metas {
        let state = kernel
            .workflow_state_bytes("com.acme/Keyed@1", Some(&meta.key_bytes))
            .unwrap()
            .expect("cell state");
    ...
    let root = kernel
        .workflow_index_root("com.acme/Keyed@1")
        .expect("persisted index root after snapshot");
