...
pub fn trace_get<S: Store + 'static>(
    ...
    let root_domain = root_domain.ok_or_else(|| {
        if let Some(hash) = event_hash.clone() {
            KernelError::Query(format!("trace root event_hash '{hash}' not found"))
        } else {
            KernelError::Query("trace root event not found for correlation query".into())
        }
    })?;
    let root_seq =
        root_seq.ok_or_else(|| KernelError::Query("trace root sequence missing".into()))?;
    let root_record_json = serde_json::to_value(&root_domain)
        .map_err(|e| KernelError::Query(format!("encode root event record: {e}")))?;
        ...
        .take(limit)
    {
        let record: JournalRecord = serde_cbor::from_slice(&entry.payload)
            .map_err(|e| KernelError::Query(format!("decode journal record: {e}")))?;
        if let JournalRecord::EffectReceipt(receipt) = &record
            && !matches!(receipt.status, ReceiptStatus::Ok)
        {
        ...
        window.push(json!({
            ...
            "record": serde_json::to_value(record)
                .map_err(|e| KernelError::Query(format!("encode journal record: {e}")))?,
        }));
    }
    ...
    let queued_effects = pending_workflow_receipts
        .iter()
        .filter_map(|effect| {
    ...
    let non_terminal_workflow_instances = workflow_instances
        ...
        .filter(|instance| {
            !matches!(
                instance.status,
                crate::snapshot::WorkflowStatusSnapshot::Completed
                    | crate::snapshot::WorkflowStatusSnapshot::Failed
            )
        })
        .count();
            ...
            "workflow_instances": workflow_instances.into_iter().map(|instance| {
                json!({
                    ...
                    "status": match instance.status {
                        crate::snapshot::WorkflowStatusSnapshot::Running => "running",
                        crate::snapshot::WorkflowStatusSnapshot::Waiting => "waiting",
                        crate::snapshot::WorkflowStatusSnapshot::Completed => "completed",
                        crate::snapshot::WorkflowStatusSnapshot::Failed => "failed",
                    },
...
pub fn workflow_trace_summary<S: Store + 'static>(
    kernel: &Kernel<S>,
) -> Result<Value, KernelError> {
...
pub fn workflow_trace_summary_with_routes<S: Store + 'static>(
    kernel: &Kernel<S>,
    route_diagnostics: Option<&TraceRouteDiagnostics>,
) -> Result<Value, KernelError> {
    ...
    for entry in kernel.dump_journal()? {
        ...
        match record {
            ...
            JournalRecord::Governance(governance) => match governance {
                ...
                crate::journal::GovernanceRecord::Approved(_) => approved += 1,
                crate::journal::GovernanceRecord::Applied(_) => applied += 1,
            },
            _ => {}
        }
    }
    ...
    for instance in &workflow_instances {
        match instance.status {
            crate::snapshot::WorkflowStatusSnapshot::Running => running += 1,
            crate::snapshot::WorkflowStatusSnapshot::Waiting => waiting += 1,
            crate::snapshot::WorkflowStatusSnapshot::Completed => completed += 1,
            crate::snapshot::WorkflowStatusSnapshot::Failed => failed += 1,
        }
...
fn first_intent_hash(value: &Value) -> Option<String> {
    let read = |arr: Option<&Vec<Value>>| -> Option<String> {
        ...
        for item in arr {
            ...
            if let Some(hashes) = item.get("intent_hashes").and_then(|v| v.as_array())
                && let Some(hash) = hashes.first().and_then(|v| v.as_str())
            {
                return Some(hash.to_string());
            }
        }
    ...
    let workflow_hash = value
        .get("workflow_instances")
        .and_then(|v| v.as_array())
        .and_then(|instances| {
            for instance in instances {
                let Some(inflight) = instance.get("inflight_intents").and_then(|v| v.as_array())
                else {
                    continue;
                };
