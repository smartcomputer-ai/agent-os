...
use aos_node::{
    EffectRuntimeEvent, HostControl, SubmissionEnvelope, SubmissionPayload, SubmissionRejection,
    WorldId, WorldLogFrame, classify_effect_kind, partition_for_world,
...
use super::core::{
    AssignmentDelta, CompletedSlice, DurableDisposition, IngressToken, LocalInputMsg,
...
impl HostedWorkerCore {
    pub(super) fn ingress_backlog_by_partition(&self) -> std::collections::BTreeMap<u32, usize> {
        self.state
            .scheduler
            .pending_by_partition
            .iter()
            .map(|(partition, queue)| (*partition, queue.len()))
            .collect()
    }

    pub(super) fn dispatch_embedded_ingress_messages(&mut self) -> Result<(), WorkerError> {
        if self.infra.kafka.is_broker() {
            return Ok(());
        }
        let Some(scheduler_tx) = self.scheduler_tx.clone() else {
            return Ok(());
        };
        let assigned = (0..self.infra.kafka.partition_count()).collect::<Vec<_>>();
        let assignment = crate::kafka::AssignmentSync {
            assigned: assigned.clone(),
            newly_assigned: Vec::new(),
            revoked: Vec::new(),
        };
        if self.assignment_delta_is_observable(&assignment) {
            let _ = scheduler_tx.send(SchedulerMsg::Assignment(AssignmentDelta {
                assigned: assignment.assigned.clone(),
                newly_assigned: assignment.newly_assigned.clone(),
                revoked: assignment.revoked.clone(),
            }));
        }
        for partition in &assigned {
            let records = self.infra.kafka.drain_pending_ingress(*partition);
            for record in records {
                let _ = scheduler_tx.send(SchedulerMsg::Ingress(record));
            }
        }
        Ok(())
    }

    fn assignment_delta_is_observable(&self, assignment: &crate::kafka::AssignmentSync) -> bool {
        if !assignment.newly_assigned.is_empty() || !assignment.revoked.is_empty() {
            return true;
        }
        self.state
            .assigned_partitions
    ...
    pub(super) fn handle_scheduler_msg(&mut self, msg: SchedulerMsg) -> Result<bool, WorkerError> {
        match msg {
            SchedulerMsg::Ingress(record) => self.handle_ingress_record(record),
            SchedulerMsg::LocalInput(local) => self.handle_local_input_msg(local),
            SchedulerMsg::Assignment(delta) => self.apply_assignment_delta(delta),
    ...
    fn handle_local_input_msg(&mut self, local: LocalInputMsg) -> Result<bool, WorkerError> {
        ...
        if !self.world_is_assigned(local.world_id) {
            return Ok(false);
        }
    ...
    fn apply_assignment_delta(&mut self, delta: AssignmentDelta) -> Result<bool, WorkerError> {
        let next_assigned = delta.assigned.iter().copied().collect::<BTreeSet<_>>();
        let changed = self.state.assigned_partitions != next_assigned
            || !delta.newly_assigned.is_empty()
            || !delta.revoked.is_empty();
        if !changed {
            return Ok(false);
        }

        self.clear_revoked_partition_state(&delta.revoked)?;
        for partition in &delta.revoked {
            self.state.scheduler.pending_by_partition.remove(partition);
        }
        let partitions_to_recover = if !delta.newly_assigned.is_empty() {
            delta.newly_assigned.clone()
        } else {
            delta.assigned.clone()
        };
        for partition in &partitions_to_recover {
            self.infra.kafka.recover_partition_from_broker(*partition)?;
        }
        self.state.assigned_partitions = next_assigned;
        self.activate_assigned_worlds()?;
        let active_world_ids = self
            .state
            .active_worlds
            .keys()
            .copied()
        ...
        self.state
            .ready_worlds
            .retain(|world_id| active_world_ids.contains(world_id));
        Ok(true)
    }

    fn world_is_assigned(&self, world_id: WorldId) -> bool {
        self.state
            .assigned_partitions
            .contains(&partition_for_world(
                world_id,
    ...
    pub(super) fn drive_scheduler_until_quiescent(
        ...
        loop {
            ...
            progressed |= self.drain_background_projection_updates()?;
    ...
    fn clear_revoked_partition_state(&mut self, revoked: &[u32]) -> Result<(), WorkerError> {
        if revoked.is_empty() {
            return Ok(());
        }

        let revoked = revoked.iter().copied().collect::<BTreeSet<_>>();
        let partition_count = self.infra.kafka.partition_count();
        let revoked_world_ids = self
            .state
            .active_worlds
            .keys()
            .copied()
            .filter(|world_id| revoked.contains(&partition_for_world(*world_id, partition_count)))
            .collect::<BTreeSet<_>>();
        let slice_ids = self
            .state
            .scheduler
            ...
            .filter_map(|(slice_id, slice)| {
                let source_revoked = slice
                    .source
                    .is_some_and(|token| revoked.contains(&token.partition));
                let world_revoked = slice.affected_worlds.iter().any(|world_id| {
                    revoked.contains(&partition_for_world(*world_id, partition_count))
                });
                (source_revoked || world_revoked).then_some(*slice_id)
            })
            .collect::<Vec<_>>();

        self.remove_slice_tracking(&slice_ids);
        for slice_id in slice_ids {
            let Some(slice) = self.state.scheduler.staged_slices.remove(&slice_id) else {
                continue;
            };
            for world_id in slice.affected_worlds {
                ...
                if let Some(world) = self.state.active_worlds.get_mut(&world_id) {
                    Self::remove_pending_slice_id(world, slice_id);
                    world.running = false;
                }
            }
        }
        self.state
            .ready_worlds
            .retain(|world_id| !revoked_world_ids.contains(world_id));
        Ok(())
    }

    fn activate_assigned_worlds(&mut self) -> Result<(), WorkerError> {
        let assigned = self.state.assigned_partitions.clone();
        let partition_count = self.infra.kafka.partition_count();
        let revoked_world_ids = self
            .state
            .active_worlds
            .keys()
            .copied()
            .filter(|world_id| !assigned.contains(&partition_for_world(*world_id, partition_count)))
            .collect::<Vec<_>>();
        for world_id in revoked_world_ids {
        ...
        self.state.active_worlds.retain(|world_id, _| {
            assigned.contains(&partition_for_world(*world_id, partition_count))
        });
        self.state.async_worlds.retain(|world_id, async_state| {
            let keep = assigned.contains(&partition_for_world(*world_id, partition_count));
            if !keep {
                async_state.abort_all_timers();
            }
            keep
        ...
        let default_universe_id = self.infra.default_universe_id;
        let world_ids = self.infra.kafka.world_ids();
        for world_id in world_ids {
            let partition = partition_for_world(world_id, partition_count);
            if !assigned.contains(&partition) {
                continue;
            }
            self.ensure_registered_world(default_universe_id, world_id)?;
    ...
    fn handle_ingress_record(&mut self, record: IngressRecord) -> Result<bool, WorkerError> {
        let token = IngressToken {
        ...
        self.route_ingress_record(token, record.envelope)?;
        Ok(true)
    }

    pub(super) fn route_ingress_record(
        &mut self,
        token: IngressToken,
        ...
        self.enqueue_ingress_item(world_id, WorkItem::Ingress { token, envelope })
    }

    fn route_host_control_ingress(
        &mut self,
        token: IngressToken,
        envelope: SubmissionEnvelope,
        control: HostControl,
        ...
        match control {
            HostControl::CreateWorld { mut request } => {
                ...
                self.stage_completed_slice(CompletedSlice {
                    ...
                    source: Some(token),
                    original_item: Some(WorkItem::Ingress { token, envelope }),
                    frames: vec![frame],
                    disposition: None,
    ...
    fn front_mailbox_item_requires_strict_quiescence(world: &super::types::ActiveWorld) -> bool {
        matches!(
            world.mailbox.front(),
            Some(WorkItem::Ingress { envelope, .. })
                if matches!(
                    envelope.payload,
    ...
    fn service_one_world(&mut self, world_id: WorldId) -> Result<(), WorkerError> {
        ...
        let accepted_submission_id = if let WorkItem::Ingress { envelope, .. } = &item {
            let submission_id = envelope.submission_id.clone();
            let already_seen = {
        ...
        let service_result: Result<(), WorkerError> = match &item {
            WorkItem::Ingress { envelope, .. } => match &envelope.payload {
    ...
    fn build_completed_slice(
        ...
        if frames.is_empty() && !matches!(item, WorkItem::Ingress { .. }) {
            return Ok(None);
        }

        let disposition = match (&item, frames.is_empty()) {
            (WorkItem::Ingress { token, envelope }, true) => {
        ...
        Ok(Some(CompletedSlice {
            ...
            staged_at: Instant::now(),
            source: match &item {
                WorkItem::Ingress { token, .. } => Some(*token),
    ...
    fn failure_slice(
        ...
        let WorkItem::Ingress { token, envelope } = &item else {
            return Ok(None);
        };
    ...
    pub(super) fn stage_completed_slice(
        ...
        if let Some(token) = slice.source {
            self.mark_pending_serviced(token, slice.id)?;
        }
        for world_id in &slice.affected_worlds {
            if let Some(world) = self.state.active_worlds.get_mut(world_id) {
                world.running = false;
    ...
    fn slice_is_barrier(slice: &CompletedSlice) -> bool {
        ...
        match slice.original_item.as_ref() {
            Some(WorkItem::Ingress { envelope, .. }) => match &envelope.payload {
...
mod tests {
    ...
    use crate::infra::kafka::IngressRecord;
    use crate::worker::HostedWorkerRuntime;
    use crate::worker::core::CheckpointCommit;
    ...
    fn enqueue_ingress(
        ...
        core.handle_ingress_record(IngressRecord {
            partition,
            offset,
            envelope: SubmissionEnvelope {
