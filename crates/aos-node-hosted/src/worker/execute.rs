...
use super::commands::{
    command_failed_record, command_running_record, command_succeeded_record, run_plane_command,
    synthesize_queued_command_record,
};
use super::types::{
    ActiveWorld, CommandRollbackRecords, CommitCommandRecords, HostedWorkerRuntimeInner,
...
impl HostedWorkerRuntimeInner {
    ...
    pub(super) fn process_existing_submission(
        ...
        if let SubmissionPayload::Command { command } = &submission.payload {
            let running_base = rollback_record
                .clone()
                .unwrap_or_else(|| synthesize_queued_command_record(command));
            rollback_command_records
                .entry((world_key, command.command_id.clone()))
                .or_insert_with(|| running_base.clone());
            self.infra
                .blob_meta_for_domain_mut(universe_id)?
                .put_command_record(world_key, command_running_record(&running_base))?;
        }
