...
impl Default for HostedSecretConfig {
    fn default() -> Self {
        Self {
            ...
            kek_bytes: decode_kek_hex(DEFAULT_UNSAFE_KEK_HEX).expect("default dev KEK must decode"),
...
impl HostedSecretConfig {
    pub fn from_env() -> Result<Self, String> {
        ...
        if let Ok(value) = std::env::var("AOS_HOSTED_KEK_ID") {
            let trimmed = value.trim();
            if !trimmed.is_empty() {
                config.kek_id = trimmed.to_string();
            }
        }
        if let Ok(value) = std::env::var("AOS_HOSTED_KEK_HEX") {
            config.kek_bytes = decode_kek_hex(&value)?;
        }
        if let Ok(value) = std::env::var("AOS_HOSTED_SECRET_ENV_FALLBACK") {
...
#[derive(Debug, Clone)]
pub struct HostedSecretPutResult {
    pub version: SecretVersionRecord,
}

#[derive(Clone)]
pub struct HostedSecretService<P> {
    persistence: Arc<P>,
    universe: UniverseId,
    config: HostedSecretConfig,
}

impl<P> HostedSecretService<P>
where
    P: SecretStore + 'static,
{
    pub fn new(persistence: Arc<P>, universe: UniverseId, config: HostedSecretConfig) -> Self {
        Self {
            persistence,
            universe,
            config,
    ...
    pub fn put_secret_value(
        ...
        actor: Option<String>,
        created_at_ns: u64,
    ) -> Result<HostedSecretPutResult, String> {
        if !matches!(
            binding.source_kind,
        ...
        Ok(HostedSecretPutResult { version })
    }
}

#[derive(Clone)]
pub struct HostedSecretResolver<P> {
    persistence: Arc<P>,
    universe: UniverseId,
    config: HostedSecretConfig,
}

impl<P> HostedSecretResolver<P>
where
    P: SecretStore + 'static,
{
    pub fn new(persistence: Arc<P>, universe: UniverseId, config: HostedSecretConfig) -> Self {
...
impl<P> SecretResolver for HostedSecretResolver<P>
where
    P: SecretStore + 'static,
{
    fn resolve(
        ...
        let value = match binding {
            ...
            Some(_) => {
                return Err(SecretResolverError::NotFound(binding_id.to_string()));
            }
...
pub fn default_kek_fingerprint(config: &HostedSecretConfig) -> String {
