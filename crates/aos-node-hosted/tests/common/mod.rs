...
use aos_node::{
    BackendError, CreateWorldRequest, CreateWorldSource, PartitionCheckpoint, UniverseId,
};
...
use aos_node_hosted::kafka::{BrokerKafkaIngress, KafkaConfig};
use aos_node_hosted::{
    HostedWorker, HostedWorkerRuntime, HostedWorldSummary, WorkerSupervisorHandle,
};
...
impl BrokerRuntimeTestContext {
    pub fn worker_runtime(&self, label: &str) -> HostedWorkerRuntime {
        self.worker_runtime_in_universe(label, aos_node::local_universe_id())
    }
    ...
    pub fn control_runtime(&self, label: &str) -> HostedWorkerRuntime {
        self.control_runtime_in_universe(label, aos_node::local_universe_id())
    }
    ...
    pub fn direct_worker_runtime(&self, label: &str, partitions: &[u32]) -> HostedWorkerRuntime {
        self.direct_worker_runtime_in_universe(label, aos_node::local_universe_id(), partitions)
    }

    pub fn direct_worker_runtime_in_universe(
        &self,
        label: &str,
        universe_id: UniverseId,
        partitions: &[u32],
        ...
        self.runtime_with_kafka(label, universe_id, kafka_config)
    }
    ...
    fn runtime_with_kafka(
        ...
        .expect("open broker hosted runtime")
    }
}

pub fn embedded_runtime(partition_count: u32) -> HostedWorkerRuntime {
    HostedWorkerRuntime::new_embedded(partition_count).expect("embedded hosted runtime")
}
...
pub async fn wait_for_worker(supervisor: &mut WorkerSupervisorHandle) {
    supervisor
        ...
        .await
        .expect("background hosted worker stays alive");
}
...
pub async fn wait_for_checkpoint(
    ...
    while Instant::now() < deadline {
        ...
        if let Some(checkpoint) = runtime
            .latest_checkpoint(world.universe_id, world.effective_partition)
            .unwrap()
...
pub fn ensure_hosted_test_env_loaded() {
    ...
    ONCE.call_once(|| {
        let _ = aos_node_hosted::load_dotenv_candidates();
    });
}
...
pub fn broker_kafka_config(label: &str, _partition_count: u32) -> Option<KafkaConfig> {
    ...
    config.submission_group_prefix = format!("aos-node-hosted-tests-{unique}");
    config.transactional_id = format!("aos-node-hosted-tests-{unique}");
    Some(config)
}
...
pub fn broker_blobstore_config(label: &str) -> Option<BlobStoreConfig> {
    ...
    config.prefix = format!("aos-node-hosted-tests/{label}/{}", uuid::Uuid::new_v4());
    Some(config)
}
...
pub fn worker_config() -> HostedWorkerConfig {
    HostedWorkerConfig {
        ...
        projection_commit_mode: aos_node_hosted::config::ProjectionCommitMode::Background,
...
fn worker_kafka_config_for_label(ctx: &BrokerRuntimeTestContext, label: &str) -> KafkaConfig {
    ...
    kafka_config.submission_group_prefix =
        format!("{}-worker", kafka_config.submission_group_prefix);
...
pub async fn wait_for_kafka_assignment(
    ingress: &mut BrokerKafkaIngress,
) -> Result<Vec<u32>, BackendError> {
    let deadline = Instant::now() + TEST_WAIT_TIMEOUT;
    while Instant::now() < deadline {
        let assigned = ingress.poll(&Default::default())?.assignment.assigned;
        if !assigned.is_empty() {
            return Ok(assigned);
        }
        tokio::time::sleep(TEST_WAIT_SLEEP).await;
    }
    Ok(ingress.assigned_partitions())
}

pub async fn wait_for_kafka_pending_submissions(
    ingress: &mut BrokerKafkaIngress,
    expected: usize,
) -> Result<Vec<aos_node_hosted::kafka::IngressRecord>, BackendError> {
    let deadline = Instant::now() + TEST_WAIT_TIMEOUT;
    while Instant::now() < deadline {
        let batch = ingress.poll(&Default::default())?;
        if batch.records.len() >= expected {
            return Ok(batch.records);
        }
        tokio::time::sleep(TEST_WAIT_SLEEP).await;
    }
    Err(BackendError::Persist(aos_node::PersistError::backend(
        format!("timed out waiting for {expected} pending submissions"),
    )))
}
...
pub fn counter_world_root() -> PathBuf {
    ...
    ROOT.get_or_init(|| authored_smoke_world_root("00-counter", "aos-node-hosted-counter-tests"))
        .clone()
}
...
pub fn timer_world_root() -> PathBuf {
    static ROOT: OnceLock<PathBuf> = OnceLock::new();
    ROOT.get_or_init(|| {
        authored_smoke_world_root("01-hello-timer", "aos-node-hosted-hello-timer-tests")
    })
    .clone()
}
...
pub fn fetch_notify_world_root() -> PathBuf {
    static ROOT: OnceLock<PathBuf> = OnceLock::new();
    ROOT.get_or_init(|| {
        authored_smoke_world_root("03-fetch-notify", "aos-node-hosted-fetch-notify-tests")
    })
    .clone()
}
...
pub fn workspace_world_root() -> PathBuf {
    static ROOT: OnceLock<PathBuf> = OnceLock::new();
    ROOT.get_or_init(|| {
        authored_smoke_world_root("09-workspaces", "aos-node-hosted-workspace-tests")
    })
    .clone()
}
