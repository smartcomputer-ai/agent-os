...
use aos_node::FsCas;
use aos_node::{
    BlobBackend, CborPayload, CheckpointBackend, CommandRecord, CommandStatus, PartitionCheckpoint,
    PromotableBaselineRef, UniverseId, WorldCheckpointRef, WorldId,
};
use aos_node_hosted::blobstore::{HostedBlobMetaStore, HostedCas, RemoteCasStore};
...
fn object_store_checkpoints_and_command_records_roundtrip() {
    ...
    blobstore.commit_checkpoint(checkpoint.clone()).unwrap();
    ...
    reopened.prime_latest_checkpoints("aos-journal", 1).unwrap();

    let latest = reopened.latest_checkpoint("aos-journal", 0).unwrap();
