use super::{
    optional_bool, parse_json_object, require_string, session_id_from_args_or_runtime, value_object,
};
use crate::contracts::ToolRuntimeContext;
use crate::tools::types::ToolMappingError;
use alloc::string::ToString;
use serde_json::{Map, Value};

pub fn map_args(
    arguments_json: &str,
    runtime: &ToolRuntimeContext,
) -> Result<Value, ToolMappingError> {
    let args = parse_json_object(arguments_json)?;
    let session_id = session_id_from_args_or_runtime(&args, runtime)?;
    let patch_text = require_string(&args, "patch")?;

    let mut out = Map::new();
    out.insert("session_id".into(), Value::String(session_id));
    out.insert(
        "patch".into(),
        Value::Object(Map::from_iter([(
            "inline_text".into(),
            Value::Object(Map::from_iter([(
                "text".into(),
                Value::String(patch_text.to_string()),
            )])),
        )])),
    );

    if let Some(dry_run) = optional_bool(&args, "dry_run") {
        out.insert("dry_run".into(), Value::Bool(dry_run));
    }

    Ok(value_object(out))
}

#[cfg(test)]
mod tests {
    use super::*;
    use aos_effects::builtins::{HostFsApplyPatchParams, HostPatchInput};

    #[test]
    fn mapped_args_decode_into_host_apply_patch_params() {
        let runtime = ToolRuntimeContext {
            host_session_id: Some("hs_1".into()),
            host_session_status: Some(crate::contracts::HostSessionStatus::Ready),
            ..ToolRuntimeContext::default()
        };
        let mapped = map_args(
            r#"{"patch":"*** Begin Patch\n*** End Patch\n","patch_format":"unified","dry_run":true}"#,
            &runtime,
        )
        .expect("map args");

        let cbor = serde_cbor::to_vec(&mapped).expect("encode mapped args");
        let decoded: HostFsApplyPatchParams =
            serde_cbor::from_slice(&cbor).expect("decode host params");
        assert_eq!(decoded.session_id, "hs_1");
        assert_eq!(decoded.patch_format, None);
        assert_eq!(decoded.dry_run, Some(true));
        match decoded.patch {
            HostPatchInput::InlineText { inline_text } => {
                assert!(inline_text.text.contains("*** Begin Patch"));
            }
            HostPatchInput::BlobRef { .. } => panic!("expected inline patch text"),
        }
    }
}
