        ...
        match input.effect_kind.as_str() {
            ...
            "host.fs.apply_patch" => {
                let params: HostFsApplyPatchParams =
                    match decode_apply_patch_params(&input.effect_params) {
                        Ok(value) => value,
                        Err(err) => return Ok(deny("effect_params_invalid", err.to_string())),
                    };
...
fn decode_cbor_value(bytes: &[u8]) -> Result<CborValue, serde_cbor::Error> {
...
fn decode_apply_patch_params(bytes: &[u8]) -> Result<HostFsApplyPatchParams, String> {
    if let Ok(params) = decode_cbor::<HostFsApplyPatchParams>(bytes) {
        return Ok(params);
    }

    let mut value = decode_cbor_value(bytes).map_err(|err| err.to_string())?;
    normalize_apply_patch_param_shape(&mut value)?;
    serde_cbor::value::from_value(value).map_err(|err: serde_cbor::Error| err.to_string())
}

fn normalize_apply_patch_param_shape(value: &mut CborValue) -> Result<(), String> {
    let CborValue::Map(obj) = value else {
        return Err("apply_patch params must be an object".to_string());
    };
    let Some(patch_value) = obj.get_mut(&CborValue::Text("patch".into())) else {
        return Err("apply_patch params missing 'patch'".to_string());
    };

    let CborValue::Map(patch_obj) = patch_value else {
        return Ok(());
    };
    let Some(CborValue::Text(tag)) = patch_obj.get(&CborValue::Text("$tag".into())) else {
        return Ok(());
    };
    let Some(payload) = patch_obj.get(&CborValue::Text("$value".into())).cloned() else {
        return Err("apply_patch tagged patch missing '$value'".to_string());
    };
    let mut normalized = BTreeMap::new();
    match tag.as_str() {
        "inline_text" => {
            normalized.insert(CborValue::Text("inline_text".into()), payload);
        }
        "blob_ref" => {
            normalized.insert(CborValue::Text("blob_ref".into()), payload);
        }
        other => {
            return Err(format!(
    ...
    *patch_value = CborValue::Map(normalized);
    Ok(())
}
