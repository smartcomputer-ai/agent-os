...
use aos_wasm_sdk::{
    ReduceError, Workflow, WorkflowCtx, Value, aos_event_union, aos_workflow, aos_variant,
};
...
aos_workflow!(BlobEchoSm);

#[derive(Default)]
struct BlobEchoSm;

impl Workflow for BlobEchoSm {
...
fn handle_start(ctx: &mut WorkflowCtx<EchoState>, event: StartEvent) {
    ...
    ctx.effects().emit_raw("sys/blob.put@1", &params, Some("default"));
}
...
fn handle_put_result(ctx: &mut WorkflowCtx<EchoState>, event: BlobPutResultEvent) {
    ...
    if let Some(stored_blob_ref) = &ctx.state.stored_blob_ref {
        ...
        ctx.effects().emit_raw("sys/blob.get@1", &params, Some("default"));
    }
}

fn handle_get_result(ctx: &mut WorkflowCtx<EchoState>, event: BlobGetResultEvent) {
