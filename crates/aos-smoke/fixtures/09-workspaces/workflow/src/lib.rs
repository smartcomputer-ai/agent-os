...
impl Workflow for WorkspaceDemo {
...
fn handle_start(ctx: &mut WorkflowCtx<WorkspaceState, ()>, start: WorkspaceStart) -> Result<(), ReduceError> {
...
fn handle_receipt(
    ctx: &mut WorkflowCtx<WorkspaceState, ()>,
...
fn maybe_begin_next_workspace(ctx: &mut WorkflowCtx<WorkspaceState, ()>) -> Result<(), ReduceError> {
    if ctx.state.active_step.is_some() {
        return Ok(());
    }
...
fn emit_empty_root(ctx: &mut WorkflowCtx<WorkspaceState, ()>) -> Result<(), ReduceError> {
    let workspace = ctx
        .state
        .active_workspace
        .clone()
...
fn emit_list(ctx: &mut WorkflowCtx<WorkspaceState, ()>, root_hash: String) -> Result<(), ReduceError> {
