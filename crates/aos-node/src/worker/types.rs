...
use aos_node::{
    BackendError, EffectRuntime, EffectRuntimeEvent, JournalBackend, JournalCommit, JournalFlush,
    RuntimeError, SharedEffectRuntime, SnapshotRecord, TimerScheduler, UniverseId, WorldConfig,
...
use crate::blobstore::{BlobStoreConfig, HostedBlobMetaStore, HostedCas, scoped_blobstore_config};
use crate::kafka::{HostedKafkaBackend, KafkaConfig, PartitionLogEntry};
use crate::sqlite::{HostedSqliteBackend, HostedSqliteJournalConfig};
...
pub enum WorkerError {
    ...
    #[error(
        "timed out waiting for durable flush for accept token {accept_token} after {timeout_ms}ms"
    )]
    WaitForFlushTimedOut { accept_token: u64, timeout_ms: u64 },
    #[error(
...
enum HostedJournalBackend {
    Kafka(HostedKafkaBackend),
    Sqlite(HostedSqliteBackend),
}

pub(super) struct HostedJournalInfra {
    backend: HostedJournalBackend,
}

impl HostedJournalInfra {
    pub fn new(partition_count: u32, config: KafkaConfig) -> Result<Self, BackendError> {
        Ok(Self {
            backend: HostedJournalBackend::Kafka(HostedKafkaBackend::new(partition_count, config)?),
        })
    }

    pub fn new_embedded(partition_count: u32, config: KafkaConfig) -> Result<Self, BackendError> {
        Ok(Self {
            backend: HostedJournalBackend::Kafka(HostedKafkaBackend::new_embedded(
                partition_count,
                config,
            )?),
        })
    ...
    pub fn new_sqlite(paths: &aos_node::LocalStatePaths) -> Result<Self, BackendError> {
        Ok(Self {
            backend: HostedJournalBackend::Sqlite(HostedSqliteBackend::new(
                HostedSqliteJournalConfig::for_paths(paths),
            )?),
        })
    }
    ...
    pub fn kafka_config(&self) -> Option<&KafkaConfig> {
        match &self.backend {
            HostedJournalBackend::Kafka(inner) => Some(inner.config()),
            HostedJournalBackend::Sqlite(_) => None,
        }
    }

    pub fn partition_count(&self) -> u32 {
        match &self.backend {
            HostedJournalBackend::Kafka(inner) => inner.partition_count(),
            HostedJournalBackend::Sqlite(_) => 1,
        }
    }

    pub fn is_broker(&self) -> bool {
        matches!(&self.backend, HostedJournalBackend::Kafka(inner) if inner.is_broker())
    }

    pub fn partition_entries(&self, topic: &str, partition: u32) -> &[PartitionLogEntry] {
        match &self.backend {
            HostedJournalBackend::Kafka(inner) => inner.partition_entries(topic, partition),
            HostedJournalBackend::Sqlite(_) => &[],
        }
    }

    pub fn recover_partition_from_broker(&mut self, partition: u32) -> Result<(), BackendError> {
        match &mut self.backend {
            HostedJournalBackend::Kafka(inner) => inner.recover_partition_from_broker(partition),
            HostedJournalBackend::Sqlite(_) => Ok(()),
        }
    }

    pub fn debug_fail_next_batch_commit(&mut self) {
        match &mut self.backend {
            HostedJournalBackend::Kafka(inner) => inner.debug_fail_next_batch_commit(),
            HostedJournalBackend::Sqlite(inner) => inner.debug_fail_next_batch_commit(),
        }
    }
}

impl JournalBackend for HostedJournalInfra {
    fn refresh_all(&mut self) -> Result<(), BackendError> {
        match &mut self.backend {
            HostedJournalBackend::Kafka(inner) => JournalBackend::refresh_all(inner),
            HostedJournalBackend::Sqlite(inner) => JournalBackend::refresh_all(inner),
        }
    }

    fn refresh_world(&mut self, world_id: WorldId) -> Result<(), BackendError> {
        match &mut self.backend {
            HostedJournalBackend::Kafka(inner) => JournalBackend::refresh_world(inner, world_id),
            HostedJournalBackend::Sqlite(inner) => JournalBackend::refresh_world(inner, world_id),
        }
    }

    fn world_ids(&self) -> Vec<WorldId> {
        match &self.backend {
            HostedJournalBackend::Kafka(inner) => JournalBackend::world_ids(inner),
            HostedJournalBackend::Sqlite(inner) => JournalBackend::world_ids(inner),
        }
    }

    fn durable_head(&self, world_id: WorldId) -> Result<WorldDurableHead, BackendError> {
        match &self.backend {
            HostedJournalBackend::Kafka(inner) => JournalBackend::durable_head(inner, world_id),
            HostedJournalBackend::Sqlite(inner) => JournalBackend::durable_head(inner, world_id),
        }
    }

    fn world_frames(&self, world_id: WorldId) -> Result<Vec<WorldLogFrame>, BackendError> {
        match &self.backend {
            HostedJournalBackend::Kafka(inner) => JournalBackend::world_frames(inner, world_id),
            HostedJournalBackend::Sqlite(inner) => JournalBackend::world_frames(inner, world_id),
        }
    }
    ...
    fn world_tail_frames(
        ...
        match &self.backend {
            HostedJournalBackend::Kafka(inner) => {
                JournalBackend::world_tail_frames(inner, world_id, after_world_seq, cursor)
            }
            HostedJournalBackend::Sqlite(inner) => {
                JournalBackend::world_tail_frames(inner, world_id, after_world_seq, cursor)
            }
        }
    }
    ...
    fn commit_flush(&mut self, flush: JournalFlush) -> Result<JournalCommit, BackendError> {
        match &mut self.backend {
            HostedJournalBackend::Kafka(inner) => JournalBackend::commit_flush(inner, flush),
            HostedJournalBackend::Sqlite(inner) => JournalBackend::commit_flush(inner, flush),
        }
    }
}

#[derive(Default)]
pub(super) struct HostedCheckpointInfra {
    by_domain: BTreeMap<UniverseId, HostedBlobMetaStore>,
}

impl HostedCheckpointInfra {
    pub fn backend_for_domain_mut(
        &mut self,
        blobstore_config: &BlobStoreConfig,
        _journal: &HostedJournalInfra,
        universe_id: UniverseId,
    ) -> Result<&mut HostedBlobMetaStore, WorkerError> {
        if !self.by_domain.contains_key(&universe_id) {
            let scoped = scoped_blobstore_config(blobstore_config, universe_id);
            let mut backend = HostedBlobMetaStore::new(scoped)?;
            backend.prime_latest_checkpoints()?;
            self.by_domain.insert(universe_id, backend);
        }
        let backend = self
            .by_domain
            .get_mut(&universe_id)
...
pub(super) enum AcceptFlushWaitState {
    #[default]
    Pending,
    Completed,
}
...
pub(super) struct AcceptFlushWaiter {
    state: Mutex<AcceptFlushWaitState>,
    ready: Condvar,
}

impl AcceptFlushWaiter {
    pub fn complete(&self) {
        let mut state = self.state.lock().expect("accept flush waiter poisoned");
        *state = AcceptFlushWaitState::Completed;
        self.ready.notify_all();
    }

    pub fn wait(&self, timeout: Option<std::time::Duration>) -> bool {
        let mut state = self.state.lock().expect("accept flush waiter poisoned");
        if *state == AcceptFlushWaitState::Completed {
            return true;
        }
        match timeout {
            Some(timeout) => {
                let (state_after, result) = self
                    .ready
                    .wait_timeout_while(state, timeout, |state| {
                        *state != AcceptFlushWaitState::Completed
                    })
                    .expect("accept flush waiter poisoned");
                state = state_after;
                *state == AcceptFlushWaitState::Completed && !result.timed_out()
                    || *state == AcceptFlushWaitState::Completed
            }
            None => {
                state = self
                    .ready
                    .wait_while(state, |state| *state != AcceptFlushWaitState::Completed)
                    .expect("accept flush waiter poisoned");
                *state == AcceptFlushWaitState::Completed
            }
        }
...
pub(super) struct HostedWorkerState {
    pub owned_worlds: BTreeSet<WorldId>,
    ...
    pub accept_waiters: BTreeMap<u64, Arc<AcceptFlushWaiter>>,
    pub next_accept_token: u64,
