...
impl HostedWorkerCore {
    pub(super) fn configure_owned_worlds(&mut self, owned_worlds: BTreeSet<WorldId>) {
        self.state.owned_worlds = owned_worlds;

        let unowned_active = self
            .state
            .active_worlds
            .keys()
            .copied()
            .filter(|world_id| !self.state.owned_worlds.contains(world_id))
            .collect::<Vec<_>>();
        for world_id in unowned_active {
        ...
        self.state
            .async_worlds
            .retain(|world_id, _| self.state.owned_worlds.contains(world_id));
        self.state
            .registered_worlds
            .retain(|world_id, _| self.state.owned_worlds.contains(world_id));
        self.state
        ...
        self.state
            .ready_worlds
            .retain(|world_id| self.state.owned_worlds.contains(world_id));
    }
    ...
    pub(super) fn ensure_registered_world(
        ...
        if !self.state.owned_worlds.contains(&world_id) {
        ...
        JournalBackend::refresh_world(&mut self.infra.journal, world_id)
    ...
    fn universe_id_from_journal(&self, world_id: WorldId) -> Option<UniverseId> {
        JournalBackend::world_frames(&self.infra.journal, world_id)
            .ok()?
            .iter()
    ...
    fn build_active_world(
        ...
        let checkpoint = self.checkpoint_world_record(universe_id, world_id);
        let (active_baseline, tail_frames, has_checkpoint, frame_count) =
            self.baseline_and_tail_frames(universe_id, world_id, checkpoint.as_ref())?;
        let replay_frame_count = tail_frames.len();
        let next_world_seq = JournalBackend::durable_head(&self.infra.journal, world_id)
            .map_err(WorkerError::LogFirst)?
            .next_world_seq
    ...
    fn baseline_and_tail_frames(
        ...
        universe_id: UniverseId,
        world_id: WorldId,
        checkpoint: Option<&aos_node::WorldCheckpointRef>,
    ) -> Result<(SnapshotRecord, Vec<WorldLogFrame>, bool, usize), WorkerError> {
        if let Some(checkpoint) = checkpoint {
            let checkpoint_snapshot = snapshot_record_from_checkpoint(&checkpoint.baseline);
            let tail_frames = JournalBackend::world_tail_frames(
                &self.infra.journal,
                world_id,
                checkpoint.baseline.height,
                checkpoint.journal_cursor.as_ref(),
            )
            .map_err(WorkerError::LogFirst)?;
            let snapshot = snapshot_record_from_frames(&tail_frames, |_| true)
                .filter(|journal| journal.height >= checkpoint_snapshot.height)
                .unwrap_or_else(|| checkpoint_snapshot.clone());
            let tail = tail_frames
                .into_iter()
                .filter(|frame| frame.world_seq_end > snapshot.height)
                .collect::<Vec<_>>();
            let frame_count = tail.len();
            return Ok((snapshot, tail, true, frame_count));
        }

        let frames = JournalBackend::world_frames(&self.infra.journal, world_id)
            .map_err(WorkerError::LogFirst)?;
        ...
        let tail = frames
            ...
            .filter(|frame| frame.world_seq_end > snapshot.height)
            .collect::<Vec<_>>();
        Ok((snapshot, tail, false, frame_count))
    }

    fn checkpoint_world_record(
        &mut self,
        universe_id: UniverseId,
        world_id: WorldId,
    ) -> Option<aos_node::WorldCheckpointRef> {
        let entry = self
            .infra
            ...
            .ok()??;
        (entry.universe_id == universe_id && entry.world_id == world_id).then_some(entry)
    }
    ...
    fn checkpoint_world_entry(
        ...
        let entry = self.checkpoint_world_record(universe_id, world_id)?;
