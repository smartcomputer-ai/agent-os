...
impl WorkerSupervisor {
    ...
    async fn serve_forever_with_scheduler(
        ...
        if let Some(owned_worlds) = &self.config.owned_worlds {
            self.runtime
                .configure_owned_worlds(owned_worlds.iter().copied())?;
        }
    ...
    async fn ensure_running(&mut self) -> Result<(), WorkerError> {
        ...
        match join.await {
            Ok(Ok(())) => Err(WorkerError::BackgroundBuild(
                "node worker exited before shutdown was requested".to_owned(),
            )),
            Ok(Err(err)) => Err(err),
            Err(err) => Err(WorkerError::BackgroundBuild(format!(
                "node worker join failed: {err}"
            ))),
        }
    }
