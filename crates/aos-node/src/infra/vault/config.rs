    ...
    fn default() -> Self {
        Self {
            ...
            kek_bytes: decode_kek_hex(DEFAULT_UNSAFE_KEK_HEX)
                .expect("default node dev KEK must decode"),
...
impl HostedSecretConfig {
    pub fn from_env() -> Result<Self, String> {
        ...
        if let Ok(value) = env::var("AOS_NODE_KEK_ID") {
            let trimmed = value.trim();
            if !trimmed.is_empty() {
                config.kek_id = trimmed.to_owned();
            }
        }
        if let Ok(value) = env::var("AOS_NODE_KEK_HEX") {
            config.kek_bytes = decode_kek_hex(&value)?;
        }
        if let Ok(value) = env::var("AOS_NODE_SECRET_ENV_FALLBACK") {
...
fn decode_kek_hex(value: &str) -> Result<[u8; 32], String> {
    let bytes = hex::decode(value.trim())
        .map_err(|err| format!("invalid node KEK hex '{}': {err}", value.trim()))?;
    <[u8; 32]>::try_from(bytes.as_slice())
        .map_err(|_| "node KEK must decode to exactly 32 bytes".to_string())
}
