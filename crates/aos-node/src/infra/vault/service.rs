...
use aos_cbor::Hash;
use aos_node::{
    LocalStatePaths, SecretBindingRecord, SecretBindingSourceKind, SecretBindingStatus,
    SecretVersionRecord, SecretVersionStatus, UniverseId,
};
...
impl HostedVault {
    ...
    ) -> Result<Self, HostedVaultError> {
        let config = HostedSecretConfig::from_env().map_err(HostedVaultError::Invalid)?;
        let backend =
            VaultBlobstore::new_persistent(unscoped_blobstore_config(&blobstore_config), paths)?;
        Ok(Self {
            config,
            backend: Arc::new(Mutex::new(backend)),
        })
...
mod tests {
    ...
    #[test]
    fn persistent_local_vault_survives_reopen() {
        let temp = tempfile::tempdir().expect("tempdir");
        let paths = LocalStatePaths::new(temp.path().join(".aos-node"));
        let config = BlobStoreConfig {
            bucket: None,
            ..BlobStoreConfig::default()
        };
        let universe_id = UniverseId::nil();
        let binding_id = "llm/openai_api";
        let secret = b"test-openai-key";

        let vault = HostedVault::new_persistent(config.clone(), &paths).expect("create vault");
        vault
            .upsert_binding(
                universe_id,
                binding_id,
        ...
        vault
            .put_secret_value(universe_id, binding_id, secret, None, Some("test".into()))
            .expect("put secret value");
        drop(vault);

        let reopened = HostedVault::new_persistent(config, &paths).expect("reopen vault");
        ...
        let resolved = reopened
            .resolve_secret_value(universe_id, binding_id, 1)
            .expect("resolve persisted secret");
