...
use aos_node::{BackendError, BlobBackend, PersistError, UniverseId};
...
impl Store for HostedCas {
    ...
    fn put_blob(&self, bytes: &[u8]) -> StoreResult<Hash> {
        self.put_verified(bytes)
            .map_err(|err| err.into_store_error(".aos/cas", "hosted CAS entry missing"))
    }
...
fn backend_to_store_error(err: BackendError) -> StoreError {
    backend_to_persist_error(err).into_store_error(".aos/cas", "hosted CAS entry missing")
}
