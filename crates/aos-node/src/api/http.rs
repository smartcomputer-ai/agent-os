...
use crate::api::{
    ...
    StateListResponse, SubmitEventBody, TraceQuery, TraceSummaryQuery, UniverseQuery,
    UpsertSecretBindingBody, WorkspaceAnnotationsQuery, WorkspaceApplyRequest,
...
async fn create_world<B: HttpBackend>(
    ...
    Ok((StatusCode::CREATED, Json(backend.create_world(body)?)))
}
...
async fn events_post<B: HttpBackend>(
    ...
    Ok((
        StatusCode::ACCEPTED,
        Json(backend.submit_event(parse_world_id(&world_id)?, body)?),
    ))
}
...
async fn receipts_post<B: HttpBackend>(
    ...
    Ok((
        StatusCode::ACCEPTED,
        Json(backend.submit_receipt(parse_world_id(&world_id)?, body)?),
    ))
}
...
async fn governance_propose<B: HttpBackend>(
    ...
    Ok(Json(
        backend.governance_propose(parse_world_id(&world_id)?, body)?,
    ))
}
...
async fn governance_shadow<B: HttpBackend>(
    ...
    Ok(Json(
        backend.governance_shadow(parse_world_id(&world_id)?, body)?,
    ))
}
...
async fn governance_approve<B: HttpBackend>(
    ...
    Ok(Json(
        backend.governance_approve(parse_world_id(&world_id)?, body)?,
    ))
}
...
async fn governance_apply<B: HttpBackend>(
    ...
    Ok(Json(
        backend.governance_apply(parse_world_id(&world_id)?, body)?,
    ))
}
...
pub fn decode_b64(value: &str) -> Result<Vec<u8>, ControlError> {
    use base64::Engine;
    base64::engine::general_purpose::STANDARD
        .decode(value)
        .map_err(|err| ControlError::invalid(format!("decode base64: {err}")))
}
