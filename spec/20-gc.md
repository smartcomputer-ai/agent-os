...
**Status**: future protocol note. The active runtime does not yet implement mark/sweep deletion GC.
This document exists to keep roots, edges, and snapshot safety compatible with a future GC pass.
...
- safe future mark/sweep deletion in local and server deployments
- shared-CAS operation across many worlds

Current scope is groundwork only. It defines roots, edge semantics, and baseline safety. It does not
require deletion GC in the active runtime.
...
- Physical storage layout is orthogonal to this model: a logical CAS object may be stored directly
  or inside an immutable pack object without changing reachability semantics.
...
Packing does not introduce a third source of logical edges. Pack membership and locator metadata
are storage artifacts, not semantic references in the object graph.

## `blob.put@1` and `BlobEdge` Semantics

The reachability-safe blob protocol is:
...
- `manifest_hash`
- workflow module state roots (workflow ABI state bytes)
- keyed workflow `cell_index_root` roots
- workspace roots (directly or via workflow-owned state roots)
...
- Implementations may treat `receipt_horizon_height == height` as the promotable baseline shape.
...
For shared CAS, server GC should mark over the union of retained roots across all worlds before sweeping.
...
Deletion mechanics are implementation-defined and are out of scope for this draft.

If an implementation uses packed physical storage, it must preserve the same logical mark result.
Pack objects and locator records are retained or collected according to the logical objects they
back; they do not change which logical hashes are considered reachable.

## Responsibility Split

### System Responsibility

Kernel/protocol surfaces MUST ensure system-defined typed payloads expose refs in traversable form. This includes snapshot completeness, manifest linkage, workflow state roots, cell index roots, and workspace roots.
...
## Workflows, Cells, and Workspaces

- Workflow state is traversable when schema-known and rooted from snapshots.
- Keyed workflows are rooted via `cell_index_root`; index nodes lead to per-cell state blobs/nodes.
- Workspace content is retained via workspace roots reachable from snapshot roots.

If workflow/workspace payloads hide hashes inside opaque bytes without explicit refs, those hidden references are out of contract and may be collected.

## Future Implementation Requirements and Non-Goals

Required outcomes before deletion GC is enabled:
