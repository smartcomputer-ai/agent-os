...
export function DefDetailPage() {
  ...
  if (kind === "plan" && name) {
    return <Navigate to={`/explorer/plans/${encodeURIComponent(name)}`} replace />;
  }
  ...
  return (
    <div className="space-y-6 animate-in fade-in-0 slide-in-from-bottom-2">
      ...
      <header className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        ...
        <div className="flex gap-2 shrink-0">
          ...
          <Button asChild variant="secondary" size="sm">
            <Link to={`/explorer/manifest?tab=${kind}`}>
      ...
      <div className="grid gap-4 lg:grid-cols-[1.3fr_0.7fr]">
        <Card className="bg-card/80">
          <CardHeader>
            <CardTitle className="text-lg">Definition content</CardTitle>
        ...
        <div className="space-y-4">
          <Card className="bg-card/80">
            ...
            <CardContent className="space-y-3 text-sm">
              <DefSummary def={def} kind={kind} name={name} />
            </CardContent>
          </Card>
        </div>
      </div>
