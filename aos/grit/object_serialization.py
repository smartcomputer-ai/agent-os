...
def is_object_id_match(object_id_a:ObjectId, object_id_b:ObjectId):
    if object_id_a is None or object_id_b is None:
        raise ValueError("Both object ids need to be set.")
    if not len(object_id_a) == _ID_LEN:
        raise ValueError("object_id_a must be 32 bytes.")
    if not len(object_id_b) == _ID_LEN:
        raise ValueError("object_id_b must be 32 bytes.")
    return object_id_a == object_id_b
