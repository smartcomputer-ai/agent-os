...
async def create_genesis_message(store:ObjectStore, sender_id:ActorId, wit_name:str) -> MailboxUpdate:
    ...
    #genesis_ref here means that the wit should be called for the genesis message instead of the default genesis handler in the actor_executor
    # this is usually the assumption of most tests (that they handle the genesis message themselves)
    gen_core:TreeObject = Core.from_external_wit_ref(wit_name, genesis_ref=wit_name)
