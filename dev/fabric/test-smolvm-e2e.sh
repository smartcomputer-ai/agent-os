...
test_bin="$(
  cargo test -p fabric-host --features smolvm-runtime --test smolvm_e2e --no-run 2>&1 \
    | tee /dev/stderr \
    | sed -n 's/^  Executable tests\/smolvm_e2e.rs (\(.*\))$/\1/p' \
    | tail -n 1
)"

if [[ -z "$test_bin" ]]; then
  echo "failed to locate compiled smolvm_e2e test binary" >&2
  exit 1
fi
...
if [[ ! -x "$test_bin" ]]; then
  echo "compiled smolvm_e2e test binary is not executable: $test_bin" >&2
  exit 1
fi
