...
- Governance correctness: capability/policy counters must settle consistently.
...
- Shared counters: rate limits are shared state; parallel decrements need a serial commit.
...
- Committer (single-threaded): applies results strictly in ticket order; re-checks policy/invariants and counters at commit; writes the journal; enqueues effects; updates state.
...
- Hash keys into K lanes; each lane has its own scheduler, counters, and (optional) sub-journal; lanes run on separate threads.
- Cross-lane communication via events (like separate worlds).

Trade-offs
- Simpler concurrency but splits global policy/counters; global queries/snapshots must stitch lanes.
...
- Fault injection: re-validation failures, adapter delays, counter edge cases.
...
- v1/v1.1: keep the kernel single-threaded. Add small parallel-ready hooks (event_id, RNG seeding, commit-time counters, journal fields).
