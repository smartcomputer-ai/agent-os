# p4: Cell Caching (Hot/Cold) and Snapshot-Anchored Persistence
...
Implemented:
- snapshot-anchored base+delta persistence in the kernel
- workflow state updates no longer write a new state blob and persistent `CellIndex` root on every event
- head reads and `list_cells` merge snapshot/base state with in-memory delta
- snapshot commit materializes delta into persisted state blobs and `CellIndex` roots
- oversized or pressure-evicted dirty delta entries spill to CAS and retain `state_hash` in delta
- local and hosted now both benefit because the behavior lives in the shared kernel hot-state model
- local snapshot cadence now follows the shared maintenance policy instead of snapshotting on every progressed pump
- shared snapshot maintenance default was raised from `256` to `10_000` journal entries

Deferred:
- expose lightweight cell-cache metrics somewhere operator-visible
  - `delta_count`
  - `resident_bytes_total`
  - `spill_put_blob_count`
  - `snapshot_put_blob_count`
- multiple persistence modes (`WriteThrough`, `SnapshotCommit`, `SnapshotOnlyNoSpill`)
- runtime pin-set machinery
- GC integration beyond the existing snapshot/baseline model
- richer provenance/debug surfaces for every state read
- partial-replay or per-key replay acceleration
- elaborate cache policy work beyond the current simple spill-on-evict policy
- time-based eviction policies
- aggressive re-residentization of spilled entries on reads

Nothing else is required right now for correctness or the current perf target.
...
The original keyed-cell implementation behaved like a *write-through cache*:
- on every reducer step that changed state, the kernel wrote the new state bytes to CAS and updated a persistent `CellIndex` root.
- cache eviction was cheap because the canonical state was already in CAS.

That was conceptually simple, but it had two problems at scale:
...
This p4 design aims to move to a *snapshot-anchored* persistence model:
...
Deferred for now:
- the implementation has already committed to snapshot-anchored persistence as the only live mode
- carrying multiple runtime-selectable persistence modes does not add enough value right now
...
2) `SnapshotCommit` (this p4 design; default for large scale)
...
This document focuses on the now-implemented `SnapshotCommit` model.
...
Only a lightweight first slice of this is worth doing now.
...
Implemented in a simple first form:
- dirty delta entries spill when they exceed a per-cell threshold
- resident dirty delta bytes spill under a global byte-pressure cap using an access-order heuristic
- spilled delta entries remain readable by `state_hash`

Still deferred:
- more advanced time-based or pressure-integrated policies
- re-residentizing spilled entries aggressively on every read
...
- `list_cells` merge is implemented
- per-read provenance is deferred
...
Implemented so far:
- replay identity across the new snapshot-anchored model is covered by kernel/runtime/smoke validation
- eviction correctness is covered by spill/readability tests for oversized dirty keyed state

Deferred:
- explicit randomized snapshot-commit ordering test
- explicit CAS write-accounting test that asserts “at most one blob per snapshot interval”
...
1) Implement snapshot-anchored base+delta persistence in the kernel. Completed.
2) Validate correctness across local and hosted hot-world flows. Completed.
3) Add spill-on-evict plus minimal metrics. Completed.
   The spill policy and internal counters exist; operator-visible metrics exposure is deferred.
4) Re-profile and only then decide whether more cache policy work is justified. Completed.
   The big perf regression is gone, keyed and non-keyed are now in the same throughput regime, and further cache-policy work is deferred.
