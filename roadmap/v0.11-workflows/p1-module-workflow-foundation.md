...
- [x] Scope 2.1: Generic receipt envelope path.
- [x] Scope 2.2: Envelope field contract completed.
- [x] Scope 2.3: Deterministic intent identity includes origin instance identity (runtime derivation added).
- [x] Scope 2.4: Receipt routing keyed by `(origin_module_id, origin_instance_key)` without subscriptions.
- [x] Scope 2.5: Typed timer/blob envelopes moved to optional helpers only.
- [x] Scope 2.1.1: Persisted workflow instance waiting model.
- [x] Scope 2.1.2: Deterministic `running <-> waiting` transitions.
- [x] Scope 2.1.3: Deterministic `last_processed_event_seq` advancement.
...
- [x] Scope 4.1: Add no-plan orchestration fixture.
- [x] Scope 4.2: Add replay check for no-plan fixture.

### Work Items by Crate
- [x] `crates/aos-kernel/world/event_flow.rs`: single-effect guard removed; deterministic per-tick limits added.
- [x] `crates/aos-kernel/receipts.rs`: generic receipt event encoding path.
...
- [x] `crates/aos-kernel/world/mod.rs` + `world/snapshot_replay.rs`: pending receipt routing identity persist/restore.
- [x] `crates/aos-kernel/world/mod.rs` + `snapshot.rs`: persisted workflow waiting metadata model.
...
- [x] `spec/defs` + `crates/aos-air-types`: adjust built-in effect origin scopes.
- [x] `spec/defs` + `crates/aos-air-types`: validate `effects_emitted` against known effect kinds.
- [x] `crates/aos-smoke` / `crates/aos-host/tests`: add no-plan orchestration fixture + replay assertions.

### Acceptance Criteria
- [x] AC1: Non-trivial workflow executes without `defplan`.
- [x] AC2: Workflow includes multiple module-emitted effects.
- [x] AC3: Replay from genesis deterministic for new fixture.
- [x] AC4: Caps/policies gate every effect intent.
- [x] AC5: Similar concurrent effects do not cross-deliver receipts.
- [x] AC6: Receipt routing correctness independent of subscription wiring changes.
- [x] AC7: Workflow lifecycle status persisted/restored on replay.
