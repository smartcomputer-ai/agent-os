...
4) Object/blob/manifest/defs surfaces: build `obj ls/get/stat` atop `ObjectCatalog` + `blob get` safeguards, implement blob put/get/stat UX defaults, and wire `manifest get/stat` + `defs get` from control/batch with meta where applicable. **(done for blob/manifest/defs; obj UX still to confirm)**
