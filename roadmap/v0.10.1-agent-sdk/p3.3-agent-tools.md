...
This document is tool-only. Prompt concerns remain in `p3.2-agent-ws.md`.
...
3. keep legacy scan fallback only as a migration bridge, then remove.

This gives low friction while keeping deterministic lookup.

## Migration Plan

1. Move tool-specific design/implementation tracking from `p3.2` to this doc.
2. Keep existing workspace catalog flow working.
3. Add direct `tool_refs` config path for no-workspace agents.
4. Introduce deterministic binding coverage checks in SDK/Demiurge reducers.
...
2. Descriptor source can be workspace or direct config refs.
3. Binding/authority stays outside workspace and is enforced.
4. At least one smoke fixture demonstrates direct tool refs with no workspace dependency.
