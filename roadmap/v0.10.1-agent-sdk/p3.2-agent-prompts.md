...
No workspace sync/apply event is needed for this path.
...
Convention-first fallback remains valid:
...
1. Keep existing workspace prompt behavior unchanged.
2. Add direct prompt ref fields to SDK contracts/schemas.
3. Update prompt materialization helper:
...
2. SDK supports prompt inputs via workspace and direct config refs.
3. At least one smoke scenario runs with direct prompt refs and no workspace binding.
4. Existing workspace smoke flow continues to pass replay parity.
