...
**Stage**: P2.6  
**Status**: Complete (closed on 2026-02-14)  
...
7. Opt-in live Agent SDK smoke commands landed in `aos-smoke`:
   - `aos-smoke chat-live` (default OpenAI Responses),
   - `aos-smoke chat-live --provider anthropic`,
   - `aos-smoke agent-live` (SDK session-event loop + iterative tool traversal),
   - `aos-smoke agent-live --provider anthropic`,
   - `chat-live` flow uses secret-injected API keys (`defsecret`-based),
   - `agent-live` uses `aos.agent/*` schemas and `aos-agent-sdk` reducer/helpers with multi-step tool batches,
   - both validate primary + follow-up turns and end with replay verification.
8. Final P2 closure report was published:
   - see `roadmap/v0.10-agent-sdk/p2-closure-report.md`.

## Post-Close Follow-Ups (Non-Blocking)

1. Optional live-provider SDK lane follow-up:
   - current smoke lane covers `openai/*` and `anthropic/*` via `chat-live` and `agent-live`,
   - add `openai-compatible/*` coverage if required in this stage.
2. Repository-level boundary guard for `sys/Llm*` ownership:
   - intentionally deferred by decision for P2 close; track as follow-up hardening.
...
4. Repo conformance check for `sys/Llm*` ownership boundary (deferred follow-up).
...
7. Repository boundary guard is explicitly deferred as a non-blocking follow-up for this close.
8. P2 closure report is published and all other stage exit criteria remain green.
