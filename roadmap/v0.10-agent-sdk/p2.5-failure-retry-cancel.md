...
P2.1 remains the base source of truth for lifecycle and in-flight cancellation fences; this stage hardens taxonomy and retry ownership around that contract.
...
### Cancellation sequence (conformance; must match P2.1 contract)

1. Host command accepted and journaled with `command_id`.
2. Reducer transitions to `Cancelling` and bumps both `session_epoch` and `step_epoch`.
