...
**Stage**: P2.2  
**Status**: In Progress (mapper + deterministic boundary coverage expanded)  
**Depends on**: P2.1
...
1. Add/confirm provider-adapter integration coverage that mapper-emitted runtime refs round-trip through provider request execution paths.
2. Add explicit deterministic `unknown provider/model` run-start failure coverage under SDK-driven flow (no partial activation).
...
Status note (2026-02-13): Deliverable #4 is implemented and mapper-level acceptance coverage is expanded; remaining work is provider-path integration completion and explicit unknown provider/model start-failure coverage.
