...
**Stage**: P2.2  
**Status**: Complete (verified on 2026-02-14)  
**Depends on**: P2.1
...
1. Removed provider-profile/profile-lookup scaffolding from `aos-agent` contracts, AIR assets, and reducer bins.
...
4. Build status:
   - `cargo check -p aos-agent` passes.
5. Implemented SDK deterministic mapper helpers:
   - `RunConfig + StepContext -> sys/LlmGenerateParams@1`
   - includes reasoning effort enum-to-text mapping and required-field validation.
6. Added deterministic SDK coverage:
   - mapper unit tests (success + validation failures),
   - run-start validation tests and smoke replay slice (`aos-smoke agent-session`).
7. Expanded deterministic acceptance coverage:
   - verifies `provider_options_ref`/`response_format_ref` survive mapper materialization into core params,
   - verifies omitted optional runtime controls stay unset (`max_tokens`, `reasoning_effort`, runtime refs) for deterministic defaulting behavior.

8. Added deterministic adapter integration coverage for runtime-ref roundtrip:
   - verifies `provider_options_ref` + `response_format_ref` materialize into provider request body in `crates/aos-host/tests/adapters_llm_integration.rs`.
9. Added deterministic SDK catalog-preflight failure coverage:
   - `apply_session_event_with_catalog` rejects unknown provider/model before partial activation,
   - unit assertions verify no state mutation on rejected run start.
10. Closed remaining deterministic P2.2 assertion set by combining:
    - mapper-materialization unit coverage (`aos-agent`),
    - adapter request-path integration coverage (`aos-host`),
    - replay-stable SDK smoke coverage (`aos-smoke agent-session`).
...
Status note (2026-02-14): provider-path integration coverage and explicit unknown provider/model start-failure coverage are implemented; deterministic P2.2 closure assertions are complete.
