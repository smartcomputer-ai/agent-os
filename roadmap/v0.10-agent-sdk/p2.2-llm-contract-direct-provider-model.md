...
1. Added P2.2 LLM contract scaffolding in SDK/core integration paths.
2. Added Rust contract scaffolding:
   - `crates/aos-agent-sdk/src/contracts/llm.rs`
3. Added core-side typed params/receipt structs used by host adapters:
...
1. Remove profile-lookup scaffolding from SDK assets and contracts.
2. Finalize core-side `sys/Llm*` schema evolution in:
   - `spec/defs`,
   - `aos-effects`,
   - `aos-host`,
   - cap enforcers/tests.
3. Implement deterministic mapper flow from SDK run/step state to core `sys/LlmGenerateParams@1`.
4. Add deterministic integration coverage for multiple providers with identical contracts.
