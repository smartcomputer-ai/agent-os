...
P2.1 remains the source of truth for base lease, batch, and pause/resume/cancel contracts; this stage adds limits/policy on top.
...
5. Long-run liveness policy over the P2.1 lease contract:
   - lease timeout thresholds,
   - deterministic lease-expiry reaction policy.
6. Parallel tool round pressure policy over the P2.1 settle contract:
   - backpressure behavior as in-flight counts approach configured ceilings.
...
5. Liveness policy helpers built on P2.1 lease primitives.
6. Backpressure helpers built on P2.1 batch-settle primitives.
