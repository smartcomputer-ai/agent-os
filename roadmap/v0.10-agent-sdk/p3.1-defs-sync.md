...
3. `manifest_loader` loads multiple AIR sources:
   ...
   - zero or more import sources (manifest nodes ignored; defs merged).
...
2. `manifest` nodes in imports are ignored (only the primary world manifest is used).
...
SDK defs are imported directly from:

1. `crates/aos-agent/air`

No parallel `air/exports/*` mirror is required.

## World Migrations Done

### `crates/aos-smoke/fixtures/22-agent-live`

1. Added `aos.sync.json` with cargo import of `aos-agent` AIR root.
2. Deleted local duplicate `air/schemas.air.json`.
3. Updated smoke harness to pass import roots when loading fixture AIR.

### `worlds/demiurge`

1. Added `air.imports` cargo entry in `worlds/demiurge/aos.sync.json`.
2. Added `cargo.manifest_path` to point at workspace root (`../../Cargo.toml`) so package discovery includes `aos-agent`.
3. Added imported schema ref (`aos.agent/SessionId@1`) in `worlds/demiurge/air/manifest.air.json` to exercise import path.
...
5. `cargo run -p aos-cli -- --quiet -w worlds/demiurge push --dry-run`
6. `cargo run -p aos-cli -- --quiet -w crates/aos-smoke/fixtures/22-agent-live push --dry-run`
7. `cargo run -p aos-cli -- --quiet -w worlds/demiurge status`
...
1. `air.imports[].lock` validation is implemented in P1 plan-import work (warn locally, fail in CI/strict mode).
...
3. Importing full AIR roots may pull in defs a world does not use yet; this is acceptable in greenfield but may need pruning rules later.
