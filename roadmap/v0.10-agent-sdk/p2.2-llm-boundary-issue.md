...
**Date**: 2026-02-13  
**Status**: Closed (completed on 2026-02-14)

Closure note (2026-02-14): all cleanup tasks and resume checklist items are complete, with integration coverage in place across SDK mapper tests, host adapter request-path tests, and deterministic smoke replay.
...
Early scaffold work blurred this boundary by defining `sys/Llm*` schemas inside `crates/aos-agent` AIR assets. That made SDK assets appear authoritative for core `sys/*` contracts.
...
In `crates/aos-agent/air/schemas.air.json`, temporary drafts introduced `sys/Llm*` schema definitions. That is now treated as invalid ownership and removed.
...
4. [x] Add integration tests proving:
...
Status note (2026-02-14): consolidated integration assertions are in place across `aos-agent` mapper tests, `aos-host` adapter request-path integration tests, and `aos-smoke` deterministic replay coverage.
