...
1. Remove any SDK-local `sys/Llm*` schema ownership.
2. Remove provider-profile and profile-lookup contracts from SDK scope.
3. Keep SDK manifest focused on direct session/run contracts.
4. Add/keep SDK-side mapper helpers:
   - `RunConfig + StepContext -> sys::LlmGenerateParams`
5. If richer low-level fields are needed, evolve them in core:
...
1. Clean SDK AIR assets to remove profile/catalog schemas and reducer routing.
2. Implement core-side `sys/Llm*` evolution in `spec/defs` + runtime crates.
3. Implement SDK mapper flow on top.
