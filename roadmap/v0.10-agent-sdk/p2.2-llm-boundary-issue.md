...
**Date**: 2026-02-13  
**Status**: Open design issue (documented for handoff)
...
2. Add an explicit section to `p2.2-provider-profiles-llm-contract.md` clarifying ownership boundary.
3. Implement core-side `sys/Llm*` evolution (if still desired) in `spec/defs` + runtime crates.
4. Implement SDK mapper and reducer event flow on top.
