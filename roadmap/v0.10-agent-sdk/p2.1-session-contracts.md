...
2. Added P2.1 contract schema assets:
   - `crates/aos-agent/air/schemas.air.json`
   - includes all listed P2.1 `aos.agent/*` core schemas.
3. Added SDK module and manifest assets:
   - `crates/aos-agent/air/module.air.json`
   - `crates/aos-agent/air/manifest.air.json`
4. Added Rust contract types and helper scaffolding:
   - `crates/aos-agent/src/contracts/*`
   - `crates/aos-agent/src/helpers/*`
5. Added isolated WASM module bins (pattern aligned to `crates/aos-sys`):
   - `crates/aos-agent/src/bin/session_workflow.rs`
   - `crates/aos-agent/src/bin/session_event_echo.rs`
6. Build status:
...
8. Added deterministic P2.1 coverage:
   ...
   - smoke fixture `20-agent-session` with replay-parity verification.
...
10. Expanded smoke coverage in `20-agent-session`:
...
11. Added dedicated lease-heartbeat renewal smoke coverage:
   - `LeaseHeartbeat` keeps run active through pre-deadline checks,
   - deterministic cancellation only after renewed heartbeat deadline passes.
12. Added high-contention multi-batch fan-in smoke coverage:
   - out-of-order settlements,
   - duplicate receipt idempotency,
   - stale receipt classification (`IgnoredStale`) under heavy ordering permutations.
...
6. Add crate scaffolding:
   - `crates/aos-agent` with shared types and reducer helper traits.
   - keep SDK reducer/pure WASM modules in this crate (`src/bin/*`) for v0.10.
   - keep reusable SDK AIR assets under `crates/aos-agent/air/`.
7. Define the P2.1 execution harness boundary:
   ...
   - do not create a separate e2e runner in `aos-agent`.
...
2. `aos-agent` crate with core IDs and state/event types, lifecycle helpers, queue semantics, and settle helpers.
...
Status note (2026-02-14): implementation satisfies all exit criteria (#1-#8), including dedicated heartbeat-renewal coverage and high-contention multi-batch fan-in smoke assertions.
