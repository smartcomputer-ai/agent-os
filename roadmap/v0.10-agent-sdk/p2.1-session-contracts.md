...
1. Complete full lifecycle/control behavior across all listed semantics (`steer`, `follow_up`, lease heartbeat expiry path, and boundary timing rules).
...
4. Expand P2.1 smoke coverage beyond the current no-tool lifecycle slice to include concurrency/cancel/stale-receipt scenarios.
...
Status note (2026-02-13): current implementation satisfies the no-tool lifecycle/replay slice (#1, #7, #8) and leaves tool-batch/stale-receipt/lease-boundary criteria open.
