...
1. Implement full lifecycle/control business logic (not just scaffold transitions/helpers).
2. Implement deterministic stale receipt handling and epoch fence behavior end-to-end.
3. Implement active tool-batch settle semantics with deterministic result ordering.
4. Add P2.1 smoke fixtures and replay-parity assertions in `crates/aos-smoke`.
...
`SessionConfig@1` is profile-first mutable defaults for future runs.
`RunConfig@1` is the resolved immutable snapshot for one active run and includes concrete
provider/model values derived at `RunStarted`.
...
2. `RunRequested.run_overrides` may replace those defaults for the next run.
3. On `RunStarted`, reducer snapshots `active_run_config` only after successful profile lookup resolution (see P2.2 dedicated profile-catalog flow).
...
Profile-first constraint:

- Session and run request configuration choose `provider_profile_id`; they do not directly set provider/model.
- Provider/model are resolved once at run start via typed profile lookup events and persisted only in `active_run_config`.
- `RunRequested.run_overrides` is a full run-scoped config value, not a sparse patch.

Run-start extension boundary:

- P2.2 adds profile lookup request/result events between `RunRequested` and `RunStarted`.
- P2.1 lifecycle semantics remain unchanged: once `RunStarted` is emitted, active run config is immutable for that run.
- If profile lookup fails, run transitions to failure path without activating an immutable run config.
...
2. `aos-agent-sdk` crate with:
   ...
   - SDK reducer/pure module binaries under `src/bin/*`,
   - reusable AIR assets under `air/` for schemas/module defs/plan templates.
