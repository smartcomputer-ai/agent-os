...
1. prompt + tool workspace sync can be composed by a parent plan (for example `core_workspace_sync@1`) using `spawn_plan` + `await_plan`.
...
1. Added direct tool refs to SDK contracts/schemas:
   - `SessionConfig.default_tool_refs`
   - `RunConfig.tool_refs`
2. Implemented deterministic tool visibility precedence in LLM materialization:
   - `step.tool_refs` > `run_config.tool_refs` > workspace `tool_catalog_ref`.
3. Exported a tool-specific reusable sync plan:
   - `aos.agent/core_tool_catalog_sync_from_workspace@1`.
4. Added composed workspace sync parent:
   - `aos.agent/core_workspace_sync@1` using prompt/tool subplans.
5. Switched live fixture wrapper wiring to `core_workspace_sync@1` and updated
   policy origins to the prompt/tool subplans.
6. Kept workspace tool catalogs convention-first:
   - `tools/catalogs/<catalog>.json`.
7. Added reducer/helper/smoke coverage for direct tool refs without workspace binding.

Out of scope for this slice:

1. Generic reducer-level tool binding coverage checks remain app-specific hooks (SDK currently
   provides descriptor selection/materialization and workspace JSON shape validation).
