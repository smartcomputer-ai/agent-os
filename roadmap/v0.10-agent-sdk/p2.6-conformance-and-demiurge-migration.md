...
1. Define conformance matrix:
   - provider/model combinations: `openai/*`, `anthropic/*`, `openai-compatible/*`.
   - scenarios:
     - no-tool completion,
     - tool call roundtrip,
     ...
     - provider/model change across runs with no in-run drift,
     - boundary ownership guard (no SDK-local `sys/Llm*` schema definitions).
...
- Include fixtures proving run-start immutability (`RunRequested -> RunStarted -> fixed active_run_config`) and no in-run provider/model drift.

## Exit Criteria

1. Conformance matrix passes for all required provider/model scenarios.
...
7. Unknown provider/model deterministically fails run start with no partial run activation.
8. Provider/model updates only affect subsequent runs; active runs keep immutable `active_run_config` through completion/cancel.
9. Repository-level conformance checks confirm `sys/Llm*` ownership remains in core crates and spec assets, not in SDK AIR assets.
