...
1. `aos.agent/core_workspace_sync@1` composes prompt/tool subplans.
...
Target reusable plans in `crates/aos-agent/air/plans/`:
...
2. `aos.agent/core_workspace_sync@1`
   - orchestration-only plan that composes prompt/tool sync subplans via `spawn_plan` + `await_plan`.

Refactor direction:

1. split workspace sync into prompt and tool subplans,
...
3. Export composed `core_workspace_sync@1` as the single workspace sync parent.
...
1. Added direct prompt refs to SDK contracts/schemas:
   - `SessionConfig.default_prompt_refs`
   - `RunConfig.prompt_refs`
2. Implemented deterministic prompt precedence in materialization:
   - run direct prompt refs > workspace prompt pack ref > none
   - history refs appended oldest -> newest.
3. Added prompt-only reusable plan export:
   - `aos.agent/core_prompt_sync_from_workspace@1`.
4. Kept workspace path optional:
   - direct prompt refs run without any workspace binding/sync.
5. Added reducer/helper tests and smoke validation for direct prompt refs and precedence.
6. Added composed parent plan export:
   - `aos.agent/core_workspace_sync@1` (spawns prompt/tool subplans and merges).
7. Switched SDK live fixture wrapper wiring to composed `core_workspace_sync@1`.

The old monolithic workspace sync plan was removed in this branch.
