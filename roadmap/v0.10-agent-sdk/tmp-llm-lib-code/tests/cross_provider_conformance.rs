...
use forge_llm::{ProviderAdapter, ToolCallData, Usage};
...
impl ProviderAdapter for ScriptedAdapter {
    ...
    async fn complete(&self, request: Request) -> Result<Response, SDKError> {
        ...
        if responses.is_empty() {
            return Err(SDKError::Configuration(forge_llm::ConfigurationError::new(
...
fn response_tool_call(provider: &str, id: &str) -> Response {
    Response {
        ...
        message: forge_llm::Message {
            role: forge_llm::Role::Assistant,
            content: vec![forge_llm::ContentPart::tool_call(ToolCallData {
                id: id.to_string(),
...
fn response_text(provider: &str, text: &str, reason: &str) -> Response {
    Response {
        ...
        model: "test-model".to_string(),
        provider: provider.to_string(),
        message: forge_llm::Message::assistant(text),
        finish_reason: FinishReason {
...
async fn tool_loop_round_trips_tool_results_for_openai_and_anthropic() {
    for provider in ["openai", "anthropic"] {
        ...
        assert!(
            second
                .iter()
                .any(|message| message.role == forge_llm::Role::Tool),
