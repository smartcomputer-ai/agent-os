...
2. Profile resolver loads `ProviderProfile@1` using selected `provider_profile_id`; catalog lookup may validate compatibility and produce diagnostics only.
3. Resolver computes concrete `provider` and `model`. Optional run controls (`reasoning_effort`, `max_tokens`) come from selected run request config, with profile defaults applied only where explicitly defined by the profile contract.
...
2. Resolve through `ProviderProfile@1` defaults/constraints.
...
2. Profile resolution (run-scoped full replacement when provided, otherwise session defaults, then profile defaults/constraints + catalog validation diagnostics) works without app-level provider branching.
