...
2. SDK requires that blob to validate as `aos.agent/LlmOutputEnvelope@1` for SDK-managed flows.
3. Non-SDK worlds may use a different payload shape while still satisfying core `sys/*` receipt semantics.
