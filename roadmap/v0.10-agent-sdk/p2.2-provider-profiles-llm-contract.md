...
1. Added P2.2 contract schemas in SDK AIR assets:
   - `aos.agent/ProviderProfile@1`
   - `sys/LlmRuntimeArgs@1`
   - replaced shapes for `sys/LlmGenerateParams@1` and `sys/LlmGenerateReceipt@1`
...
1. Implement profile lookup request/resolution/failure runtime flow between session and catalog reducers.
2. Update `aos-host` LLM adapter to fully consume/pass expanded `sys/LlmGenerateParams@1` fields into `aos-llm`.
3. Implement normalized `output_ref` envelope generation and `raw_output_ref` parity behavior in adapter tests.
4. Add deterministic integration coverage (OpenAI/Anthropic profile paths, unknown profile failure path, replay parity checks).
...
4. Manifest/cap/policy updates aligned to the in-place schema replacement, including routing for profile lookup request/result events.
...
7. Unknown `provider_profile_id` path produces deterministic `ProfileLookupFailed@1` and run failure with no partial run activation.
