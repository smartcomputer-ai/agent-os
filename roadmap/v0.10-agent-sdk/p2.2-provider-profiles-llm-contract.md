...
This document is the normative source of truth for P2.2 provider/LLM contracts.
...
3. If required resolved values are missing after applying run-scoped config and profile defaults, resolver must fail with typed validation error rather than infer host-side defaults.
...
- nested `runtime`:
  - step-local execution payload (`temperature`, optional `max_tokens`, `reasoning_effort`, tool/stop/options/format refs)
...
- `runtime` groups step-local execution fields cleanly and avoids endless top-level field growth.
- `provider`, `model`, and `provider_profile_id` are sourced from immutable active run config.
- `runtime.max_tokens` is sourced from active run config when set; when unset, adapter passes no max-token override and provider defaults apply.
...
1. At run start, compute effective run request config from `RunRequested.run_overrides` when present, otherwise `session_config` defaults (P2.1).
2. Profile resolver loads `ProviderProfile@1` using effective `provider_profile_id`; catalog lookup may validate compatibility and produce diagnostics only.
3. Resolver computes concrete `provider` and `model` and applies run-scoped defaults for `reasoning_effort` and `max_tokens`.
4. Reducer snapshots immutable `active_run_config` (P2.1) containing `{provider_profile_id, provider, model, reasoning_effort?, max_tokens?}`.
5. For each LLM step, `LlmGenerateParams@1` is materialized from `active_run_config` plus deterministic step context (`message_refs`, tools, stop sequences, metadata, format refs).
6. No host/user step-level config override path exists in P2.2.
7. Capability checks still apply at effect boundary (`llm.basic` cap and policy).
8. Adapter receives concrete fully-resolved request values.
9. If required values remain unset after precedence resolution, resolver fails with typed validation error.

Run-start precedence order:

1. `RunRequested.run_overrides` (run-scoped).
2. `session_config` defaults.
3. `ProviderProfile@1` defaults.
4. No semantic defaults from host catalog/runtime.

Step materialization rule:

- Config-bearing fields (`provider_profile_id`, `provider`, `model`, `runtime.reasoning_effort`, `runtime.max_tokens`) must come from immutable `active_run_config`.
- Step-local runtime fields may vary only from deterministic reducer/plan logic for that step.

`max_tokens` rule:

- If `active_run_config.max_tokens` is set, materialize it into `runtime.max_tokens`.
- If `active_run_config.max_tokens` is unset, keep `runtime.max_tokens` unset and use provider/model default behavior.
...
2. Profile resolution (run-scoped overrides + session defaults + profile defaults + catalog validation diagnostics) works without app-level provider branching.
